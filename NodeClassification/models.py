...
class GraIncV2(torch.nn.Module):
    def __init__(self, dataset, args):
        super(GraIncV2, self).__init__()
        self.lin11 = Linear(dataset.num_features, args.hidden)
        self.lin12 = Linear(dataset.num_features, args.hidden)
        self.lin13 = Linear(dataset.num_features, args.hidden)
...
class GraInc(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.prop11 = Gra_inc(args.Inc1)
        self.prop12 = Gra_inc(args.Inc1)
        self.prop13 = Gra_inc(args.Inc1)
        self.prop21 = Gra_inc(args.Inc2)
        self.prop22 = Gra_inc(args.Inc2)
        self.prop23 = Gra_inc(args.Inc2)
    ...
    def forward(self, data):
        ...
        if self.dprate == 0.0:
            x21 = self.prop11(x11, edge_index)
            x22 = self.prop12(x12, edge_index)
            x23 = self.prop13(x13, edge_index)
            x31 = self.prop21(x21, edge_index)
            x32 = self.prop22(x22, edge_index)
            x33 = self.prop23(x23, edge_index)
            x1 = torch.cat([x11, x12, x13], dim=1)
            x2 = torch.cat([x21, x22, x23], dim=1)
            x3 = torch.cat([x31, x32, x33], dim=1)
            x = x1 + x2 + x3
